...
* [A helpful presentation on Youtube](https://www.youtube.com/watch?v=oO8n3y23b6M), and the [associated source files](https://github.com/alysivji/talks/tree/master/data-science-workflows-using-docker-containers)
...
* Create images:
  ```
  docker build -t <image_name> .  # Create image using this directory's Dockerfile
  ```
* Create containers:
  ```
  docker run -p 4000:80 <image_name>  # Run "friendlyname" mapping port 4000 to 80
  docker run -d -p 4000:80 <image_name>         # Same thing, but in detached mode
  docker run username/repository:tag                   # Run image from a registry
  ```
* Get basic info and stats:
  ```
  docker image ls -a                             # List all images on this machine
  docker container ls                                # List all running containers
  docker container ls -a             # List all containers, even those not running
  docker stats               # display a live stream of containers' resource usage
  ```
* Stop/kill/remove containers:
  ```
  docker container stop <container>      # Gracefully stop the specified container
  docker container kill <container>    # Force shutdown of the specified container
  docker container rm <container>   # Remove specified container from this machine
  ```
* Remove images:
  ```
...
### Useful commands for interaction with containers
...
1. Create an empty directory, and create `Dockerfile` inside of it:
    ...
    # Meta-data (not needed unless you want to upload the image to a registry)
    ...
    # Install a few scientific libraries
    ...
    If needed, the `Dockerfile` may refer to some external files, which need to be in the specified host directory (usually the same directory which holds the Dockerfile).
...
2. Build the Docker image:
    ...
    ## check that the built image is there
    docker image ls
    ```
